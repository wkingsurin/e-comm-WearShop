...
export default function FastViewButton({
    ...
    return (
        <Button
            className="absolute z-2 bg-white text-black pointer-events-none opacity-0 bottom-[calc(45%-30px)] border-[0.5px] border-transparent scale-90 gap-3 group-hover/card:pointer-events-auto group-hover/card:opacity-100 group-hover/card:bottom-[calc(45%-15px)] group-hover/card:scale-101 hover:shadow-[0_4px_12px_-3px_rgba(0,0,0,.15)] hover:bg-white hover:text-black"
            onClick={(e) => {
                e.preventDefault();
                e.stopPropagation();
                updateOverlay({ open: true });
                updateModal({
                    target: { product: data, variantId },
                    contentType,
                });
                addLastSeen(data.id);
            }}
